...
adminOptions:On('open', function(menu)
  ...
  menu:AddButton({
    ...
    select = function()
      TriggerServerEvent('qr-logs:server:CreateLog', 'admin', 'Admin Options', 'red', GetPlayerName() .. ' > REVIVED SELF <')
      GotoMarker()
    end
  })
  menu:AddButton({
  ...
  menu:AddButton({
    ...
    select = function()
      TriggerServerEvent('qr-logs:server:CreateLog', 'admin', 'Admin Options', 'red', GetPlayerName() .. ' toggled > INVISIBILITY <')
      if Invisible then
  ...
  menu:AddButton({
    ...
    select = function()
      Godmode = not Godmode
      TriggerServerEvent('qr-logs:server:CreateLog', 'admin', 'Admin Options', 'red', GetPlayerName() .. ' toggled > GODMODE <')
      if Godmode then
        while Godmode do
          Wait(0)
  ...
  QRCore.Functions.TriggerCallback('admin:server:hasperms', function(hasperms)
    if hasperms then
      ...
      menuSlider1:On('select', function(item, value)
        TriggerServerEvent("qr-weathersync:server:setWeather", value)
      end)

      menuSlider2:On('select', function(item, value)
        TriggerServerEvent("qr-weathersync:server:setTime", value, value)
      end)
    end
  end, 'time')
end)
